...
class SurveyState:
    ...
    @classmethod
    def new_state_object(cls, survey_instance_id, owner, next_question, timeout=3600):
        str_survey_instance_id = str(survey_instance_id)
        event_id = str_survey_instance_id + "_" + str(next_question)
        return cls(event_id, str_survey_instance_id, owner, next_question, timeout=timeout)
