...
class SurveyStateService:
    ...
    def get_by_instance(self, survey_instance_id, last_key=None):
        try:
            if last_key is None:
                response = self.dynamo.query(
                    ...
                    KeyConditionExpression="survey_instance_id = :sid AND survey_status BETWEEN :min AND :max",
            ...
            if 'Items' in response and len(response['Items']) > 0:
                ...
                return self.get_by_instance_and_status(survey_instance_id, response['LastEvaluatedKey'])
            else:
                return None
    ...
    def get_by_owner(self, survey_owner, last_key=None):
        try:
            if last_key is None:
                response = self.dynamo.query(
                    TableName=self.cache_name,
            ...
            if 'Items' in response:
                ...
                for i in response['Items']:
                    item = SurveyState.from_item(i)
                    if item.survey_status != SurveyStatus.TERMINATED_COMPLETE:
                        items.append(item.survey_instance_id)

                if 'LastEvaluatedKey' in response:
                    items += self.get_by_owner(survey_owner, response['LastEvaluatedKey'])
